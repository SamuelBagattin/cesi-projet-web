...
class Walker extends Lint.RuleWalker {
    ...
    constructor(sourceFile: ts.SourceFile, options: Lint.IOptions) {
        ...
        this._enabled = Object.keys(this._rules).length > 0 && fileGlobs.some(p => minimatch(relativeFilePath, p));
    }
    ...
    private _validatedDecorator(decorator: any) {
        ...
        const missing = Object.keys(rules.required).filter(key => !props.find((prop: any) => prop.name === key));
            ...
            Object.keys(config)
                ...
                .forEach(decoratorName => {
                    ...
                    if (typeof decoratorConfig === 'string') {
                        ...
                        output[decoratorName] = Object.keys(decoratorConfig).reduce(
                            (rules, prop) => {
                                const isForbidden = prop.indexOf('!') === 0;
